...
)
{
    ...
    half3 diffuse = Fd_Burley_Filament(roughness, NoV, NoL, LoH);

    #if defined(_SUBSURFACE_COLOR)
    // Energy conservative wrap diffuse to simulate subsurface scattering
    diffuse *= saturate((dot(N, L) + 0.5) / 2.25);
    #endif

    half3 Fd = diffuse * diffuseColor;
    ...
    // Filament
    half  D = D_Ashikhmin_Filament(roughness, NoH);
    float Vis = V_Neubelt_Filament(NoV, NoL);
    half3 F = sheenColor;

    // 由于使用了不同于标准模型的BRDF,这里不使用能量补偿
    half3 Fr = (D * Vis) * F;
    ...
    #if defined(_SUBSURFACE_COLOR)
    // 低消耗的次表面散射
    Fd *= saturate(subsurfaceColor + NoL); // 由于NoL在这里已经纳入计算,启用次表面散射后续不用再 *NoL
    half3 DirectLighting = Fd + Fr * NoL;
    
    DirectLighting *= shadow * lightColor;
    #else
    half3 DirectLighting = Fd + Fr;
    
    DirectLighting *= shadow * lightColor * NoL;
    #endif
